...
RESERVED_JSON = {"dashboards.json", "generator_dashboards.json", "timers.json", "data.json"}
...
class DebugStorageCog(commands.Cog):
    ...
    @app_commands.command(
        name="debug_storage", description="Show storage paths and the *.json files the bot can see."
    )
    ...
    async def migrate_regular_lists_to_subdir(
        self, interaction: discord.Interaction, cleanup: bool = False
    ):
        ...
        if not moved:
            return await interaction.response.send_message(
                "No regular list JSONs found to migrate.", ephemeral=True
            )
    ...
    async def migrate_gen_lists_to_volume(
        ...
        if not moved:
            return await interaction.response.send_message(
                "No generator list JSONs found to migrate.", ephemeral=True
            )
