...
class State:
    ...
    # persistence
    def save(self) -> None:
        _write_json_atomic(self.path, self._s)
    ...
    # config
    def set_cfg(self, k, v) -> None:
        self._s.setdefault("config", {})[k] = v
        self.save()

    def get_cfg(self, k, d=None):
        return self._s.get("config", {}).get(k, d)
    ...
    def summarize_rl(self) -> Dict[str, int]:
        ...
        def count_in_window(win: int) -> int:
            cutoff = now - win
            return sum(1 for t in arr if t >= cutoff)

        return {
            "15m": count_in_window(900),
            "1h": count_in_window(3600),
            "24h": count_in_window(86400),
            "total_kept": len(arr),
        }

    # boot/shutdown
    def record_boot(self, dep_id: str = "", sha: str = "", branch: str = "") -> None:
        self._s["last_boot_ts"] = time.time()
        if dep_id:
            self._s["last_deployment_id"] = dep_id
        if sha:
            self._s["last_git_sha"] = sha
        if branch:
            self._s["last_git_branch"] = branch
        self.save()

    def record_shutdown(
        self, reason: str = "unknown", dep_id: str = "", sha: str = "", branch: str = ""
    ) -> None:
...
def attach_debug_filters_and_handlers() -> None:
...
class DebugCog(commands.Cog):
    ...
    @diag.command(
        name="set_disconnect_threshold",
        description="Override disconnect alert threshold (seconds) — stored only",
    )
    ...
    async def maintenance(self, interaction: discord.Interaction, on: bool, note: Optional[str] = None):
        ...
        await interaction.response.send_message(
            f"✅ Maintenance mode {'ENABLED' if on else 'disabled'}.", ephemeral=True
        )
