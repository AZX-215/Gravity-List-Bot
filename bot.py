...
print("🔧 Global bot.py v2.3 is loading…")
...
async def delete_list_cmd(interaction: discord.Interaction, name: str):
    if not list_exists(name):
        await interaction.response.send_message(f"⚠️ List '{name}' not found.", ephemeral=True)
        return
    delete_list(name)
...
async def show_list(interaction: discord.Interaction, name: str):
    data = load_list(name)
    if not data:
        await interaction.response.send_message(f"📭 '{name}' is empty or doesn't exist.", ephemeral=True)
        return
