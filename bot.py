...
logger = logging.getLogger("gravity_list")
...
def build_embed(list_name: str) -> discord.Embed:
    ...
    # ordering: Header → Text → Timer → Items
    ...
    for it in data:
        ...
        if cat == "Header":
            ...
            embed.add_field(name=f"⏳   {it['name']} — <t:{end_ts}:R>", value="\u200b", inline=False)
        else:
...
@app_commands.describe(list_name="List to modify", index="Text line number (1-based)", new_text="Updated text")
...
@app_commands.describe(list_name="List to modify", category="Category (Owner, Enemy, Friend, etc.)", item_name="Item to add")
async def add_name(interaction: discord.Interaction, list_name: str, category: str, item_name: str):
    ...
    if category not in CATEGORY_EMOJIS:
        return await interaction.response.send_message(f"❌ Invalid category. Choose from: {', '.join(CATEGORY_EMOJIS)}", ephemeral=True)
    data = load_list(list_name)
    data.append({"category": category, "name": item_name})
    save_list(list_name, data)
    await interaction.response.send_message(f"✅ Added {CATEGORY_EMOJIS[category]} **{item_name}** to '{list_name}'.", ephemeral=True)
...
async def remove_name(interaction: discord.Interaction, list_name: str, item_name: str):
    ...
    for i, it in enumerate(data):
        if it.get("name") == item_name and it.get("category") not in ("Header", "Text", "Timer"):
            data.pop(i)
            save_list(list_name, data)
            return await interaction.response.send_message(f"✅ Removed **{item_name}**.", ephemeral=True)
...
@app_commands.describe(list_name="List to modify", old_name="Current item name", new_name="New item name")
async def edit_name(interaction: discord.Interaction, list_name: str, old_name: str, new_name: str):
    ...
    for it in data:
        if it.get("name") == old_name and it.get("category") not in ("Header", "Text", "Timer"):
            it["name"] = new_name
            save_list(list_name, data)
            return await interaction.response.send_message(f"✅ Renamed **{old_name}** → **{new_name}**.", ephemeral=True)
    await interaction.response.send_message(f"❌ Item '{old_name}' not found.", ephemeral=True)


@bot.tree.command(name="move_name", description="Move an item to a new position in a list")
@app_commands.describe(list_name="List to modify", item_name="Item to move", position="New 1-based position")
async def move_name(interaction: discord.Interaction, list_name: str, item_name: str, position: int):
    ...
    save_list(list_name, data)
    await interaction.response.send_message(f"✅ Moved **{item_name}** to position {pos}.", ephemeral=True)
...
async def sort_list(interaction: discord.Interaction, list_name: str):
    ...
    data = load_list(list_name)
    name_items = sorted(
        [it for it in data if it.get("category") not in ("Header", "Text", "Timer")],
        key=lambda x: x["name"].lower()
    )
    new_data = [it for it in data if it.get("category") in ("Header", "Text", "Timer")] + name_items
...
@app_commands.describe(list_name="List to modify", item_name="Item to comment on", comment="Comment text")
async def add_comment(interaction: discord.Interaction, list_name: str, item_name: str, comment: str):
    ...
    for it in data:
        if it.get("name") == item_name and it.get("category") not in ("Header", "Text", "Timer"):
            it["comment"] = comment
            save_list(list_name, data)
            return await interaction.response.send_message(f"✅ Comment added to **{item_name}**.", ephemeral=True)
    await interaction.response.send_message(f"❌ Item '{item_name}' not found.", ephemeral=True)


@bot.tree.command(name="edit_comment", description="Edit an existing comment")
@app_commands.describe(list_name="List to modify", item_name="Item whose comment to edit", new_comment="Updated comment text")
async def edit_comment(interaction: discord.Interaction, list_name: str, item_name: str, new_comment: str):
    ...
    for it in data:
        if it.get("name") == item_name and "comment" in it:
            ...
            save_list(list_name, data)
            return await interaction.response.send_message(f"✅ Comment updated for **{item_name}**.", ephemeral=True)
    await interaction.response.send_message(f"❌ No existing comment on '{item_name}'.", ephemeral=True)


@bot.tree.command(name="remove_comment", description="Remove a comment from an item")
@app_commands.describe(list_name="List to modify", item_name="Item whose comment to remove")
async def remove_comment(interaction: discord.Interaction, list_name: str, item_name: str):
    ...
    for it in data:
        if it.get("name") == item_name and "comment" in it:
            del it["comment"]
            save_list(list_name, data)
            return await interaction.response.send_message(f"✅ Comment removed from **{item_name}**.", ephemeral=True)
    await interaction.response.send_message(f"❌ No comment found on '{item_name}'.", ephemeral=True)
...
async def deploy_gen_list_cmd(interaction: discord.Interaction, name: str):
    if not gen_list_exists(name):
        return await interaction.response.send_message(f"❌ No generator list named '{name}'.", ephemeral=True)
